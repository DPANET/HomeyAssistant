...
exports.Validator = exports.ValidatorProviders = void 0;
...
class Validator {
    ...
    customErrorMessage() {
        return {
            ...
            "any.empty": `{#label} key value is empty, value should be within the list`,
            "date.max": `{#label} should not exceed {#limit}`,
            "any.allowOnly": `{#label} should be within the acceptable list of values`,
            "any.required": `{#label} is mandatory field`,
            "number.base": `{#label} expects integer`,
            "string.base": `{#label} expects string`,
            "array.includesOne": `{#label} expects a value not in the list`,
            "object.child": `{#label} expects a value not in the list`
        };
    }
    genericValidator(validateFn) {
        let result, err, iErr;
        err = validateFn.error;
        if (!(0, isNullOrUndefined_1.isNullOrUndefined)(err)) {
