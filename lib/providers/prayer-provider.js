...
class PrayerTimeProvider extends PrayerProvider {
    ...
    async getPrayerMethodsById(index) {
        let err, prayerMethodsList, prayerMethod;
        const filterById = ramda.where({ id: ramda.equals(index) });
        [err, prayerMethodsList] = await to(this.getPrayerMethods());
        if (err)
            return Promise.reject(PrayerErrorMessages.TIME_OUT);
        prayerMethod = ramda.filter(filterById, prayerMethodsList).pop();
        if (!util_1.isNullOrUndefined(prayerMethod))
            return prayerMethod;
        else
    ...
    async getPrayerSchoolsById(index) {
        let err, prayerSchoolsList, prayerSchool;
        const filterById = ramda.where({ id: ramda.equals(index) });
        [err, prayerSchoolsList] = await to(this.getPrayerSchools());
        if (err)
            return Promise.reject(PrayerErrorMessages.FILE_NOT_FOUND);
        prayerSchool = ramda.filter(filterById, prayerSchoolsList).pop();
        if (!util_1.isNullOrUndefined(prayerSchool))
            return prayerSchool;
        else
    ...
    async getPrayerSettings() {
        ...
        if (err || util_1.isNullOrUndefined(result))
            return Promise.reject(PrayerErrorMessages.FILE_NOT_FOUND);
        return {
            method: await this.getPrayerMethodsById(result.method),
            midnight: await this.getPrayerMidnightById(result.method),
            school: await this.getPrayerSchoolsById(result.school),
            latitudeAdjustment: await this.getPrayerLatitudeById(result.latitudeAdjustment),
            startDate: utility_1.DateUtil.formatDate(result.startDate),
            endDate: utility_1.DateUtil.formatDate(result.endDate),
            adjustments: result.adjustments
        };
    }
    async savePrayerSettings(prayerSettings) {
        let err, result;
        let db;
        db = await this.getDB();
    ...
    async getPrayerMidnightById(index) {
        let err, prayerMidnightList, prayerMidnight;
        const filterById = ramda.where({ id: ramda.equals(index) });
        [err, prayerMidnightList] = await to(this.getPrayerMidnight());
        if (err)
            return Promise.reject(PrayerErrorMessages.FILE_NOT_FOUND);
        prayerMidnight = ramda.filter(filterById, prayerMidnightList).pop();
        if (!util_1.isNullOrUndefined(prayerMidnight))
