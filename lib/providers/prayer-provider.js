...
class PrayerTimeProvider extends PrayerProvider {
    ...
    async getPrayerTime(prayerSettings, prayerLocation) {
        let duration = this.getMonthsDifference(prayerSettings.startDate, prayerSettings.endDate);
    ...
    buildPrayersObject(result) {
        ...
        let fn = n => {
            ...
            prayersList.push({
                ...
                prayersDate: moment(n.date.readable, 'DDMMMY').toDate()
            });
        };
    ...
    getMonthsDifference(startDate, endDate) {
        let startMoment = moment(startDate).startOf('M');
        let endMoment = moment(endDate).startOf('M');
        return endMoment.diff(startMoment, 'M', false) + 1;
    }
    buildPrayerAPIQueryString(url, prayerSettings, prayerLocation) {
        let queryString = {
            uri: url,
            qs: {
                uri: url,
