...
async function buildLocationObject() {
    ...
    try {
        ...
        let date = "24 Apr 2014, 04:37";
        moment_timezone_1.default(date, 'ff');
        console.log(moment_1.default.tz(`${date}`, "DD MMM YYYY, hh:mm", "Asia/Riyadh").toDate());
        console.log(moment_1.default.tz(`${date}`, "DD MMM YYYY, hh:mm", "Asia/Dubai").toDate());
...
async function pipe() {
    let date = moment_1.default.utc("2013-03-22T23:02:05Z").toDate();
    let dateAdded = moment_1.default.utc(date).subtract(120, 'm').toDate();
    let event = later_1.default.parse.recur().after(120).minute();
    let schedule = later_1.default.schedule(event);
    schedule.next(1, dateAdded);
    //let timer:later.Timer    = later.setTimeout(task,later.s)
    console.log(date);
    console.log(dateAdded);
    console.log(schedule.next(1, date));
    console.log(later_1.default.minute.next(date, 120));
