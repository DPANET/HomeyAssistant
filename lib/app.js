...
async function buildLocationObject() {
    try {
        let locationObject = await manager.LocationBuilder
            .createLocationBuilder()
            .setLocationAddress('Dubai', 'AE')
            .createLocation();
        let prayerConfig = await new cg.default().getPrayerConfig();
        let prayerSetting = await manager.PrayerSettingsBuilder
...
async function buildPrayerObject() {
    try {
        ...
        // let locationObject: loc.ILocationEntity = await locationBuilder.setLocationAddress('Dubai', 'AE').
        //     then(lb => lb.createLocation());
        let config = new cg.default();
        let prayerConfig = await config.getPrayerConfig();
        console.log(prayerConfig);
        let result = await config.savePrayerConfig(prayerConfig);
    }
    catch (err) {
