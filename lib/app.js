...
const cfgSchema = __importStar(require("./cache/schema.userscache"));
...
async function buildLocationObject() {
    ...
    try {
        ...
        let prayerDBURI = nconf_1.default.get('MONGO_DB');
        prayerDBConnection = await mongoose_1.default.connect(prayerDBURI, { useNewUrlParser: true, useUnifiedTopology: true });
        mongoose_1.default.set('useCreateIndex', true);
        let prayerConfigModel = cfgSchema.prayerTimeModel;
        let result = await prayerConfigModel.findOne();
        console.log(util_1.default.inspect(result.prayersTime.prayers, { showHidden: true, depth: null }));
        ...
        let profileID = new mongoose_1.default.Types.ObjectId("5f20ebac9627ac26ccc551e0");
        let configProvider = configuration_1.ConfigProviderFactory.createConfigProviderFactory();
        let prayerConfig = await configProvider.getPrayerConfig({ profileID: profileID });
        let locationConfig = await configProvider.getLocationConfig({ profileID: profileID });
        let config = await configProvider.getConfigId({ profileID: profileID });
...
async function pipe() {
    try {
        ...
        let validation = async (word) => {
            return word + " Funck";
            // return (word=== "Hi Space") ? true: false;
        };
        let validations = composition_1.default.railAsync(validation, y);
        let router = {
            prayerAdjust: validations,
            prayerFunc: composition_1.default.railAsync(validations, middleWare)
        };
        let z = (middleWare, next, args) => {
            let piper = composition_1.default.pipe(middleWare, next);
            return piper(args);
        };
        let subtract = (z) => {
            z * 3;
        ...
        let curry = composition_1.default.curry(x);
        let math = curry(3);
        let number = composition_1.default.pipe(math, subtract);
        console.log(number(2));
        let list = (fn, input) => {
            console.log(input);
            return fn(input);
        };
        let chainList = composition_1.default.chain(list);
