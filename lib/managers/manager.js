...
class PrayerManager {
    ...
    async updatePrayerConfig(prayerConfig, config, configProvider) {
        try {
            ...
            if (isNullOrUndefined_1.isNullOrUndefined(configProvider))
                configurator = configuration_1.ConfigProviderFactory.createConfigProviderFactory();
            else
    ...
    async updateLocationConfig(locationConfig, config, configProvider) {
        try {
