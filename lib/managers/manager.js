...
class PrayerSettingsBuilder {
    constructor(prayerProvider, validator, prayerConfig) {
        ...
        this._prayerSettings.startDate = util_1.isNullOrUndefined(prayerConfig.startDate) ? new Date() : prayerConfig.startDate;
        this._prayerSettings.endDate = util_1.isNullOrUndefined(prayerConfig.endDate) ? utility_1.DateUtil.addMonth(1, new Date()) : prayerConfig.endDate;
    }
...
class PrayerManager {
    ...
    getPrayerTime(prayerName, prayerDate) {
        let prayersByDate = this.getPrayerByDate(prayerDate);
    ...
    getUpcomingPrayer(prayerType) {
        let dateNow = utility_1.DateUtil.getNowDate();
        ...
        let todayPrayers = this.getPrayerByDate(dateNow);
    ...
    processUpcomingPrayer(dateNow, prev, curr, index, array) {
        console.log(dateNow.toLocaleDateString());
        console.log("current Date: " + curr.prayerTime.toLocaleDateString());
        if (util_1.isNullOrUndefined(prev) && curr.prayerTime >= dateNow)
            return array[index];
        else if (!util_1.isNullOrUndefined(prev) && prev.prayerTime <= dateNow && curr.prayerTime >= dateNow)
            return array[index];
        else if (!util_1.isNullOrUndefined(prev) && array.length === index + 1) {
