...
class Configurator {
    constructor(fileName) {
        if (!isNullOrUndefined_1.isNullOrUndefined(fileName))
            ...
            this._fileName = path_1.default.join(config_1.default.get("CONFIG_FOLDER_PATH"), config_1.default.get("PRAYER_CONFIG"));
    }
    ...
    async savePrayerConfig(prayerConfigs) {
        try {
            ...
            result = await this.getDB()
                .then(async (result) => {
                return await result.get(configPaths.prayerConfig)
                    .assign(updated)
                    .write();
            });
